...
touch "$MNT_POINT/sensors_config/.schema/threshold.INTEGER.ATTR.vfs2db"
touch "$MNT_POINT/sensors_output/.schema/last_value.INTEGER.ATTR.vfs2db"

mkdir "$MNT_POINT/sensors_config/1"
mkdir "$MNT_POINT/sensors_output/1"

echo "50" > "$MNT_POINT/sensors_config/1/threshold.vfs2db"
