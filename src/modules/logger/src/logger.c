...
int logger_init_default(void) {
    return logger_init(LOG_LEVEL_INFO, NULL, true, true, true, true);
}
 ...
 * @return 0 if initialization was successful, -1 otherwise.
 */
int logger_init(LogLevel level, const char* log_file_path, bool show_timestamp, bool show_location, bool show_colors, bool show_thread_id) {
    FILE* output = stderr;
...
LogLevel logger_get_level_from_string(const char* str) {
    ...
    if (strcasecmp(str, "trace") == 0) ret = LOG_LEVEL_TRACE;
    if (strcasecmp(str, "debug") == 0) ret = LOG_LEVEL_DEBUG;
    if (strcasecmp(str, "info")  == 0) ret = LOG_LEVEL_INFO;
    if (strcasecmp(str, "warn")  == 0) ret = LOG_LEVEL_WARN;
    if (strcasecmp(str, "error") == 0) ret = LOG_LEVEL_ERROR;
    if (strcasecmp(str, "fatal") == 0) ret = LOG_LEVEL_FATAL;
    if (strcasecmp(str, "off")   == 0) ret = LOG_LEVEL_OFF;
 ...
 */
void logger_write(LogLevel level, const char* file_path, int line, const char* func, const char* fmt, ...) {
    assert(g_logger.initialized && "Logger not initialized");
    ...
    if (g_logger.show_colors) {
        fprintf(out, "%s%-5s%s ", LEVEL_COLORS[level], 
                LOG_LEVEL_NAMES[level], ANSI_RESET);
    } else {
